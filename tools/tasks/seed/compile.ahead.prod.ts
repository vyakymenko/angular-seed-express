...
import { TMP_CLIENT_DIR, BOOTSTRAP_DIR } from '../../config';
...
export = (done: any) => {
  ...
  copyFile('tsconfig.json', TMP_CLIENT_DIR, join(TMP_CLIENT_DIR, BOOTSTRAP_DIR), (content: string) => {
    const parsed = JSON.parse(content);
    parsed.files.push('main.ts');
    return JSON.stringify(parsed, null, 2);
  });
  copyFile('typings.d.ts', TMP_CLIENT_DIR, join(TMP_CLIENT_DIR, BOOTSTRAP_DIR), (content: string) => {
    return content.replace('../../typings/index.d.ts', '../../../typings/index.d.ts');
  });
  const args = argv;
  const cliOptions = new tsc.NgcCliOptions(args);
  tsc.main(join(TMP_CLIENT_DIR, BOOTSTRAP_DIR), cliOptions, codegen)
    .then(done)
    .catch(e => {
